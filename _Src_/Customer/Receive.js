...
import { Divider, Checkbox, Button, List, BottomNavigation } from 'react-native-paper';
...
const Receive = ({ navigation }) => {
    ...
    async function patchsalebill(result) {
        ...
        console.log("ApiCall // salebill", "recno:", result.recno, 'receiver date:', AppFunction.getToday().dataDate, 'receiver time:', AppFunction.getTime().dataTime, "status:", "CR", "transportpickup:", result.transportpickup)

        var sendapidata = {
            "recno": result.recno,
            "receiverdate": AppFunction.getToday().dataDate,
            "receivertime": AppFunction.getTime().dataTime,
            "status": "CR",
    ...
    function filterReceivedList() {
        const result = filterBillDetails.filter(received);

        function received(item) {
            return item.isReceived == true;
        }

        console.log('filterReceivedList-->', result.length)
                                    ...
                                    <TouchableOpacity style={{ flexDirection: 'row', marginTop: '3%', marginBottom: '1%' }}>
                                        ...
                                        <View style={{ alignItems: 'center' }}>
                                            <Checkbox
                                                color={'dodgerblue'}
                                                // key={item.key}
                                                status={filterBillDetails[index].isReceived ? 'checked' : 'unchecked'}
                                                onPress={(n) => {
                                                    // console.log('n==>', n)
                                                    setfilterBillDetails((p) => {
                                                        p[index].isReceived = !p[index].isReceived;
                                                        setIndex(index)
                                            ...
                                            />
                                            {
                                                filterBillDetails[index].isReceived == false ?
                                                    <Text>Received</Text> : <Text>unreceived</Text>
                                            }
                                        </View>
