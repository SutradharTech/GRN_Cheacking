...
const ShowItem = ({route, navigation}) => {
  ...
  const {custName, From, billno, domainrecno, domainuserrecno, ApiCall} =
    route.params;
  ...
  async function getcounterbill() {
    ...
    const {data: UpdateBillData} = await axios.post(
      AppConstants.APIurl2 + 'getcounterbill/',
      sendapidata,
    );
  ...
  async function getbatchno() {
    ...
    const {data: ItemBatchData} = await axios.post(
      AppConstants.APIurl2 + 'getstockdata/',
      sendapidata,
    );
  ...
  async function resendCounterBill() {
    ...
    const {data: UpdateBillData} = await axios.post(
      AppConstants.APIurl2 + 'addcounterbill/',
      senddataapi,
    );
  ...
  async function addcounterbill() {
    console.log(
      'Api Call / addcounterbill /',
      'listHeader:',
      listHeader,
      'makerDate:',
      AppFunction.getToday().dataDate,
      'makerTime:',
      AppFunction.getTime().dataTime,
      'status:',
      'C',
    );
    let senddataapi = {
  ...
  async function counterbillfooter(item) {
    ...
    const FooterBillData = await axios.patch(
      AppConstants.APIurl2 + 'counterbillfooter/',
      senddataapi,
    );
          ...
          <Card style={styles.Body_Main_Card}>
            ...
            <View
              ...
              <Text style={{fontSize: 17}}>
                {' '}
                {item?.innerbox} X {item?.pack} X {item?.unit}
              </Text>
              <View>
                <Checkbox
                  color={'orange'}
                  // key={item.key}
                  status={
                    list[index].attributeschecked ? 'checked' : 'unchecked'
                  }
                  onPress={n => {
                    // console.log('n==>', n)
                    setlist(p => {
            ...
            <View
              ...
              <TouchableOpacity>
                <Button
                  ...
                  onPress={() => {
                    filterBatchfun(item.itemrecno, item.qty), showModal();
                  }}>
